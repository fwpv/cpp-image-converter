...
void SaveImageToScanline(const Image& out_image, int y, JSAMPLE* row) {
    const Color* line = out_image.GetLine(y);
    for (int x = 0; x < out_image.GetWidth(); ++x) {
        const Color& color = line[x];
        JSAMPLE* pixel = row + x * 3;
        pixel[0] = static_cast<JSAMPLE>(color.r);
        pixel[1] = static_cast<JSAMPLE>(color.g);
        pixel[2] = static_cast<JSAMPLE>(color.b);
    }
}
...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    std::vector<JSAMPLE> vec(row_stride, 0);
    JSAMPLE* row = vec.data();

    while (cinfo.next_scanline < cinfo.image_height) {
        int y = cinfo.next_scanline;
        SaveImageToScanline(image, y, row);
